...
root.title("SOFTWARICA CANTEEN BILLING")
root.configure(background='BLUE')

Tops = Frame(root,bg='light green',bd=20,pady=5,relief=RIDGE)
Tops.pack(side=TOP)

lblTitle=Label(Tops,font=('arial',60,'bold'),text='SOFTWARICA CANTEEN BILLING',bd=21,bg='GREY',
                fg='SKY BLUE',justify=CENTER)
lblTitle.grid(row=0)


ReceiptCal_F = Frame(root,bg='SKY BLUE',bd=10,relief=RIDGE)
ReceiptCal_F.pack(side=RIGHT)

Buttons_F=Frame(ReceiptCal_F,bg='SKY BLUE',bd=3,relief=RIDGE)
...
Drinks_F=Frame(MenuFrame,bg='light green',bd=4)
Drinks_F.pack(side=TOP)


Drinks_F=Frame(MenuFrame,bg='light green',bd=4,relief=RIDGE)
Drinks_F.pack(side=LEFT)
Food_F=Frame(MenuFrame,bg='light green',bd=4,relief=RIDGE)
...
def CostofItem():
    ...
    SubTotalofITEMS ="Rs",str('%.2f'%(PriceofDrinks + PriceofFood + 1))
    SubTotal.set(SubTotalofITEMS)

    Tax ="Rs",str('%.2f'%((PriceofDrinks + PriceofFood + 1) * 0.02))
    PaidTax.set(Tax)

    TT=((PriceofDrinks + PriceofFood + 1) * 0.02)
    TC="Rs",str('%.2f'%(PriceofDrinks + PriceofFood + 1 + TT))
    TotalCost.set(TC)
